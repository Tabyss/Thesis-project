...
import { BsPlusCircle, BsFillBarChartFill } from "react-icons/bs";import { MdEdit } from "react-icons/md";
...
function Dashboard() {
  ...
  useEffect(() => {
    const fetchInvite = async () => {
      try {
        const response = await axios.get(`http://localhost:5000/undangan/${userId}`);
        ...
        <div key={inviteItem.id} className="dash-card">
          <div className="dash-card-tumb">
            <div className="dash-card-tumb-icon">
              ...
              <Link to={`/edit/4/${inviteItem.id}`}><BsFillBarChartFill className="tumb-icon" /></Link>
            </div>
